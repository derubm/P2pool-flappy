...
function UpdateData() {
  ...
  d3.json('../local_stats', function(local_stats) {
	...
	d3.select('#time_to_share_secs').text(d3.format('.3s')(secs) +" seconds");
    ...
    d3.json('../global_stats', function(global_stats) {
	  ...
	  d3.select('#network_hashrate').text(d3.format('.3s')(global_stats.network_hashrate) + 'H/s');
      var time_to_block = local_stats.attempts_to_block/global_stats.pool_hash_rate;
	  d3.select('#time_to_block').text(d3.format('.3r')(time_to_block/60) + " minutes");
	  d3.select('#block_difficulty').text(d3.format('.3r')(global_stats.network_block_difficulty));
	  d3.select('#blockdiff').text(d3.format('.3r')(global_stats.network_block_difficulty * 65536));
